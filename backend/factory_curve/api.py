...
class FactoryCurveController:
    ...
    def get_cal_factory_curve(self, request, payload: CalPumpPayload_schema):
        ...
        return JsonResponse({"data" : pump_cal_result}, status=200)
    #    This is old version
    #    # Get query parameters
    #    print(payload)
...
#
    ...
    #                flow_f = float(flow)
    #                eff_i = int(eff.strip("%"))
    #                valid_points.append({'head': head_f, 'flow': flow_f, 'eff': eff_i})
    #            except (TypeError, ValueError):
    #                continue  
    #        
    #        flow_data = np.array([point['flow'] for point in valid_points])
    #        head_data = np.array([point['head'] for point in valid_points])
    #        efficiency = np.array([point['eff'] for point in valid_points])
#
...
#
    ...
    #            continue
    #        if imp_str != "0.00" and imp_str != "" and imp_str and flow_str and head_str :  # ✅ Correct string value comparison
...
#
    ...
    #            continue  
#
    #        if eff_str != "0.00" and eff_str != "" and eff_str and flow_str and head_str :  # ✅ Correct string value comparison
...
#
    ...
    #    #return JsonResponse({"equation": imp_grouped}, status=200)
    #    for dia in dia_key:
    #        # Filter out invalid points
    #        valid_points = []
    #        for point in imp_grouped[dia]:
    #            head = point.get('head')
    #            flow = point.get('flow')
    #            try:
    #                head_f = float(head)
    #                flow_f = float(flow)
    #                valid_points.append({'head': head_f, 'flow': flow_f})
    #            except (TypeError, ValueError):
    #                continue
    #            
    #        # Convert to arrays
    #        x = np.array([point['flow'] for point in valid_points])
    #        y = np.array([point['head'] for point in valid_points])
    #    
    #        # Check for empty data
    #        if len(x) == 0 or len(y) == 0:
    #            continue
...
#
    ...
    #                flow_f = float(flow)
    #                head_f = float(head)
    #                lower_points.append((flow_f, head_f))
    #            except (TypeError, ValueError):
    ...
    #                flow_f = float(flow)
    #                head_f = float(head)
    #                upper_points.append((flow_f, head_f))
    #            except (TypeError, ValueError):
    #                continue  # Skip invalid data
#
...
#
    ...
    #    
    #    valid_points = []
    #    for point in npshr_data:
    #        npshr = point.get('npshr')
    #        flow = point.get('flow')
    #        try:
    #            npshr_f = float(npshr)
    #            flow_f = float(flow)
    #            valid_points.append({'npshr': npshr_f, 'flow': flow_f})
    #        except (TypeError, ValueError):
    #            continue
    #        
    #    # Convert to arrays
    #    x = np.array([point['flow'] for point in valid_points])
    #    y = np.array([point['npshr'] for point in valid_points])
#
    #    npshr_best_r2 = 0
